...
const View: React.FC = () => {
  ...
  const doctorPage = [
    ...
    getItem(t('page.registrationInfo'), '/reservationInformation', <UserOutlined />),
    getItem(t('page.prescriptionInformation'), '/prescriptionManagement', <UserOutlined />),
    getItem(t('page.caseManagement'), '/caseFiling', <UserOutlined />),
    getItem(t('page.systemSettings'), '/systemSettings', <UserOutlined />, [
      getItem(t('page.personalInformationManagement'), '/personalInformationManagement', <UserOutlined />),
    ]),
  ];
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Sider breakpoint="lg" collapsedWidth={window.innerWidth < 700 ? 0 : 75} trigger={null} collapsible collapsed={collapsed} onCollapse={(value) => setCollapsed(value)}>
        <div className={style.logo} >
          ...
          <div style={collapsed ? { display: 'none' } : { height: '20px', width: '130px', overflow: 'hidden',fontSize: '12px'}}>{t('login.systemName')}</div>
        </div>
        <Menu
          theme="dark"
